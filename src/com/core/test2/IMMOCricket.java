...
public class IMMOCricket {
	...
	public static void login() {
		...
		WebElement password = driver.findElement(By.name("password:container:input"));
		password.clear();
		password.sendKeys("Ingram@1001");

		driver.findElement(By.name(":submit")).click();
		System.out.println("----------	Successfully logged in as IMMO CRICKET user 	----------");
	}
		...
		for (int i = 0; i < 400; i++) {
			try {
				...
				Boolean flag = (js.executeScript("return document.readyState").equals("complete")
						&& driver.findElement(By.linkText(element)).isEnabled());
	...
	public static void catalog() {
		...
		wait(3);
		driver.findElement(By.linkText("Apple iphone 4S-16GB-Black (Reconditioned)")).click();
		System.out.println("----------	Product selection done for purchase	----------");
		wait(2);
		WebElement quantity = driver.findElement(By.name("productQuantity:container:input"));
		quantity.clear();
		quantity.sendKeys("2");
		...
		System.out.println("----------	Product successfully added in the Shopping Cart	----------");
		// wait(2);
		...
		WebElement purchaseDate = driver.findElement(
				By.name("innerContainer:containerType:widthType:innerContainer_body:purchaseOrder:container:input"));
		purchaseDate.clear();
		purchaseDate.sendKeys("TEST" + dateFormat.format(date));
		System.out.println("----------	Date format conversion successfull	----------");
		...
		wait(2);
		driver.findElement(By.name("innerContainer:containerType:widthType:innerContainer_body:shipMethodSelection"))
				.click();

		Select selectPaymentMethod = new Select(driver.findElement(By.id("selectPaymentMethod")));
		selectPaymentMethod.selectByVisibleText("Credit Account");

		driver.findElement(By.name(
				"innerContainer:containerType:widthType:innerContainer_body:multipleConfirmationSelection:container:orderConfirmationEntry"))
	...
	public static void fetchingIMEI() {
		...
		yourOrderNumber = (driver.findElement(By.xpath("//*[@id='id147']/ul/li[2]/ul/li[3]/ul/li[2]/p[1]")).getText())
				.trim();
	...
	public static void orderHistory() {
		wait(3);
		System.out.println("----------	Fetching Order History report	----------");
		...
		Select mulOption = new Select(
				driver.findElement(By.name("border:containerType:widthType:border_body:logisticsCustomers:customers")));
		...
		wait(2);
		System.out.println("----------	Fetching Purchase Order Number for for Order Number -" + yourOrderNumber
				+ "	----------");
		wait(2);
		purchaseOrderNumber = driver
				.findElement(By.xpath("//*[@id='leftLargest']/ul/li[2]/ul/li[3]/ul/li[1]/ul/li[1]/ul[3]/li[2]"))
				.getText();
		...
		System.out.println(
				"***************     SANITY TESTING IS COMPLETED. SITE IS UP & WORKING AS EXPECTED     ***************");
	}
}
